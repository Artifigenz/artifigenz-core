...
async function maybeDetectIncome(agentInstanceId: string): Promise<void> {
  ...
  try {
    ...
    // Subscriptions run after income — they look at outflows, income
    // looks at inflows, so no overlap, but downstream Subscription
    // Radar treats subscriptions as its primary data, so we want them
    // landed by the time the brief generates.
    void maybeDetectSubscriptions(agentInstanceId);
  } catch (err) {
    console.error(`[income] failed for ${agentInstanceId}:`, err);
  }
}

const SUBSCRIPTION_THROTTLE_MS = 10 * 60_000;
const lastSubscriptionAt = new Map<string, number>();

async function maybeDetectSubscriptions(
  agentInstanceId: string,
): Promise<void> {
  const last = lastSubscriptionAt.get(agentInstanceId) ?? 0;
  if (Date.now() - last < SUBSCRIPTION_THROTTLE_MS) return;
  lastSubscriptionAt.set(agentInstanceId, Date.now());
  try {
    const stats = await detectSubscriptions(agentInstanceId);
    if (
      stats.classifiedAsSubscription > 0 ||
      stats.cacheHits > 0 ||
      stats.llmErrors.length > 0
    ) {
      const sub = Object.entries(stats.bySubtype)
        ...
        .join(" ");
      console.log(
        `[subscription] ${agentInstanceId}: ` +
          `sub=${stats.classifiedAsSubscription} not=${stats.classifiedAsNotSubscription} ` +
          `cache=${stats.cacheHits} errors=${stats.llmErrors.length} ${sub}`,
      );
    }
    // Fee/interest runs after subscription — both look at outflows. We
    // want subscription brands tagged first so the fee detector's
    // DEFINITIVE_NOT_FEE cache short-circuit can skip them.
    void maybeDetectFeeInterest(agentInstanceId);
  } catch (err) {
    console.error(`[subscription] failed for ${agentInstanceId}:`, err);
  }
}

const FEE_INTEREST_THROTTLE_MS = 10 * 60_000;
const lastFeeInterestAt = new Map<string, number>();

async function maybeDetectFeeInterest(
  agentInstanceId: string,
): Promise<void> {
  const last = lastFeeInterestAt.get(agentInstanceId) ?? 0;
  if (Date.now() - last < FEE_INTEREST_THROTTLE_MS) return;
  lastFeeInterestAt.set(agentInstanceId, Date.now());
  try {
    const stats = await detectFeeInterest(agentInstanceId);
    if (
      stats.classifiedAsFee > 0 ||
      stats.cacheHits > 0 ||
      stats.llmErrors.length > 0
    ) {
      const sub = Object.entries(stats.bySubtype)
        ...
        .join(" ");
      console.log(
        `[fee-interest] ${agentInstanceId}: ` +
          `fee=${stats.classifiedAsFee} not=${stats.classifiedAsNotFee} ` +
          `cache=${stats.cacheHits} errors=${stats.llmErrors.length} ${sub}`,
      );
    }
    // Loan EMI runs after fee_interest. Both look at outflows; running
    // fees first means pure interest postings already have a category
    // when the loan detector evaluates EMI candidates.
    void maybeDetectLoanEmi(agentInstanceId);
  } catch (err) {
    console.error(`[fee-interest] failed for ${agentInstanceId}:`, err);
  }
}

const LOAN_EMI_THROTTLE_MS = 10 * 60_000;
const lastLoanEmiAt = new Map<string, number>();

async function maybeDetectLoanEmi(agentInstanceId: string): Promise<void> {
  const last = lastLoanEmiAt.get(agentInstanceId) ?? 0;
  if (Date.now() - last < LOAN_EMI_THROTTLE_MS) return;
  lastLoanEmiAt.set(agentInstanceId, Date.now());
  try {
    const stats = await detectLoanEmi(agentInstanceId);
    if (
      stats.classifiedAsLoan > 0 ||
      stats.cacheHits > 0 ||
      stats.llmErrors.length > 0
    ) {
      const sub = Object.entries(stats.bySubtype)
        ...
        .join(" ");
      console.log(
        `[loan-emi] ${agentInstanceId}: ` +
          `loan=${stats.classifiedAsLoan} not=${stats.classifiedAsNotLoan} ` +
          `cache=${stats.cacheHits} errors=${stats.llmErrors.length} ${sub}`,
      );
    }
...
async function maybeDetectVariableRecurring(
  ...
  try {
    ...
    // Final pass: miscellaneous catches everything that didn't fit any
    // typed detector and subtypes it (dining, shopping, transport, etc.)
    // so the user can see WHERE their discretionary spend went.
    void maybeDetectMiscellaneous(agentInstanceId);
  } catch (err) {
...
const lastMiscAt = new Map<string, number>();

async function maybeDetectMiscellaneous(
  agentInstanceId: string,
): Promise<void> {
  const last = lastMiscAt.get(agentInstanceId) ?? 0;
  if (Date.now() - last < MISC_THROTTLE_MS) return;
  lastMiscAt.set(agentInstanceId, Date.now());
  try {
    const stats = await detectMiscellaneous(agentInstanceId);
    if (
      stats.classified > 0 ||
      stats.cacheHits > 0 ||
      stats.llmErrors.length > 0
      ...
      const sub = Object.entries(stats.bySubtype)
        ...
        .join(" ");
      console.log(
        `[misc] ${agentInstanceId}: ` +
          `classified=${stats.classified} cache=${stats.cacheHits} ` +
          `errors=${stats.llmErrors.length} ${sub}`,
      );
    }
  } catch (err) {
    console.error(`[misc] failed for ${agentInstanceId}:`, err);
  }
}
...
/**
 * GET /api/finance/categories/fee-interest
 *   Flat list of brands the user paid fees / interest to, sorted by
 *   total. Sample descriptions surface so the user can see why we called
 *   it a fee ("MONTHLY MAINTENANCE FEE", "INTEREST CHARGE ON PURCHASES").
 */
app.get("/categories/fee-interest", async (c) => {
  const user = c.get("user");
  const [instance] = await db
    .select({ id: agentInstances.id })
    .from(agentInstances)
...
/**
 * GET /api/finance/categories/loan-emi
 *   Flat list of lenders the user is repaying. Same brand-with-samples
 *   shape as fee-interest.
 */
app.get("/categories/loan-emi", async (c) => {
  const user = c.get("user");
  const [instance] = await db
    .select({ id: agentInstances.id })
    .from(agentInstances)
...
/**
 * GET /api/finance/categories/miscellaneous
 *   Grouped by subtype. Each subtype lists the brands under it with
 *   sample descriptions so the user can audit the categorization.
 */
app.get("/categories/miscellaneous", async (c) => {
  const user = c.get("user");
  const [instance] = await db
    .select({ id: agentInstances.id })
    .from(agentInstances)
  ...
  const rows = await db.execute<{
    ...
    FROM base b
    LEFT JOIN top_descs td USING (brand_slug)
    GROUP BY b.brand_slug, b.system_category, td.sample_descriptions
    ORDER BY ABS(SUM(b.amount::numeric)) DESC
  `);
  ...
  const subtypeMap = new Map<
    string,
    { subtype: string; label: string; total: number; brands: Brand[] }
  >();

  for (const r of rows) {
    const subtype = r.system_category ?? "other_misc";
    let bucket = subtypeMap.get(subtype);
    if (!bucket) {
      bucket = {
        subtype,
  ...
  const subtypes = Array.from(subtypeMap.values())
    .map((s) => ({ ...s, total: Math.round(s.total * 100) / 100 }))
    .sort((a, b) => {
      const ai = ordering.indexOf(a.subtype);
      const bi = ordering.indexOf(b.subtype);
